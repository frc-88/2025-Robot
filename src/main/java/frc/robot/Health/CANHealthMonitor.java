 ...
 * 
 * This singleton class provides real-time monitoring of CAN device connectivity for DEV1 base,
 * which includes swerve drive modules, gyro, and vision cameras.
 * 
 * Phase 1 Implementation: Basic device status tracking with AdvantageKit logging integration.
 */
public class CANHealthMonitor {
    ...
    /**
     * Defines the physical wiring order for each CAN bus on DEV1.
     * Device keys must match the strings passed to updateStatus().
     * 
     * IMPORTANT: Update these lists to match your actual DEV1 CAN bus configuration!
     */
    private final Map<String, List<String>> wiringOrder = Map.of(
        "rio", List.of(
            // Drive system devices - update based on your actual CAN IDs and bus configuration
    ...
    public void updateStatus(String deviceKey, boolean isConnected) {
        ...
        if (previousStatus != isConnected) {
            ...
            Logger.recordOutput("CANHealth/StatusChange/" + deviceKey.replace("/", "_") + "_timestamp", 
    ...
    /**
     * Updates the connection status for devices that use Phoenix 6 hardware objects.
     * Convenience method for devices that have isConnected() methods.
     */
    ...
    private void cleanupOldEvents(Queue<StatusEvent> events) {
        ...
        while (!events.isEmpty() && 
               (currentTime - events.peek().timestamp) > EVENT_HISTORY_DURATION) {
            events.poll();
        }
    }
    ...
    public void periodic() {
        ...
        for (Map.Entry<String, List<String>> bus : wiringOrder.entrySet()) {
            ...
            Logger.recordOutput("CANHealth/Bus_" + bus.getKey() + "/PercentConnected", 
        ...
        Logger.recordOutput("CANHealth/Overall/PercentConnected", 
